    ...
    if (population > 0) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (newY >= 0 && newY < Region.size() && newX >= 0 && newX < Region[0].size()) {
                // Spread pollution to adjacent cells, decreasing by one unit for each cell away from the source
...
void generateAndDistributeGoods(std::vector<std::vector<pune>>& Region, int x, int y, std::vector<std::vector<int>> mapKey) {
...
pune industrialProcess(pune& city, std::vector<std::vector<pune>>& Region, int x, int y) {
    ...
    if (city.getPopulation() == 0) {
        ...
        updatedCity.updatePopulation();
        // Assign available workers to the cell and adjust total
        return updatedCity;
    }
